...
public class Test4 {
   ...
   */
    ...
    int person, com, cnt=0;
    
    System.out.println("컴퓨터가 기억하는 숫자를 맞추는 게임");
    com = (int)(Math.random()*100)+1;
    
    while(true) {
      if(cnt == 5) {
        System.out.println("5번 시도하셔서 모두 틀렸습니다. 컴퓨터가 기억한 숫자는? " + com);
        System.out.println("다시 시도하시려면 0을 입력하세요!");
        person =sc.nextInt();
        if(person == 0) {
      ...
      if(person > com) {
        System.out.println("수행횟수 : "+cnt+", 더 작은수를 입력하세요..");
        continue;
      }
      else if(person < com) {
        System.out.println("수행횟수 : "+cnt+", 더 큰수를 입력하세요..");
        continue;
      }
      else {
        System.out.println("정답입니다."+cnt+", 번 만에 맞추셨군요! 축하합니다!");
        break;
      }
    }
